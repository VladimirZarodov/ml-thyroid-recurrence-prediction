...
Ноутбук `Pred_thyroid.ipynb` содержит полный пайплайн: загрузку данных, предобработку, обучение моделей и визуализацию результатов.

> ⚠️ **Важно:** проект создан в образовательных и исследовательских целях. Его результаты не являются медицинской рекомендацией. Для принятия клинических решений необходима консультация специалиста.

## 📊 Данные

Используется файл `filtered_thyroid_data.csv`, содержащий следующие признаки:
